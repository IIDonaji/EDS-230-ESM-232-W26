...
# relative difference (new - base / base)
(site2$mean-site1$mean)/site1$mean * 100

# or this way relative difference in parameter
(0.6-0.8)/0.8 * 100
```
...
**Informal** - just try varying parameters to see what it does
...
site2 <- eff %>% map(~ solarpv(area = 0.1, 
  ...
  eunit = "kWhr", g = FALSE, etype = "direct", eff = .x)) # .x substetud value with the numbers being piped
...
tmp <- map_dfr(site2, `[`, c("annual")) # map_dfr() to extract

# convert from tibble to a data frame how do you know its tibble and not a dataframe
...
ggplot(site2df, aes(year, elect, group = year)) +
  ...
  geom_line(data = site2_average, aes(year, elect, group = 1), col = "orange", lwd=2) # Mean line
```
...
-   how does the variation in parameter impact model output
    -   good to always make this graph
...
-   **Sensitivity analysis** is a way to explore how model outputs change when inputs or parameters are varied (how does model change when inputs change)
-   **Uncertainty analysis** is a way to quantify the uncertainty in model outputs due to uncertainty in inputs or parameters (the sensitivity...)
...
ethresh <- rnorm(mean = 10000, sd = 0.15, n = 20)


param <- ethresh %>% map(~ solarpv(area = 0.1, 
  solar = sierraczosolar, clr = "green", 
  eunit = "W", g = FALSE, etype = "direct",eff = 0.8,  ethresh = .x)) # .x substituted value with the numbers being piped
...
# this is pretty messy - but we can extract a useful data structure,lets say we want
# just the annual data (not the mean annual time series), and then reformat as a data frame with nice column names
tmp <- map_dfr(param, `[`, c("annual")) # map_dfr() to extract

# convert from tibble to a data frame how do you know its tibble and not a dataframe
paramdf <- data.frame(year = tmp$annual$year, elect = tmp$annual$elect)
head(paramdf)

# plot
# we also might want an average across parameter uncertainty
param_average <- paramdf %>%
  group_by(year) %>%
  dplyr::summarize(elect = mean(elect))

# now add this to the plot - note that we remove the grouping by using group=1
ggplot(paramdf, aes(year, elect, group = year)) +
  geom_boxplot() +
  labs(y = "...") +
  geom_line(data = param_average, aes(year, elect, group = 1), col = "orange", lwd=2) # Mean line
```
```
